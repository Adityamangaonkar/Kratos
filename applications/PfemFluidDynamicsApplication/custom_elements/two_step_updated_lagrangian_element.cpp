...
namespace Kratos {
								...
								const double Weight)
  {
    ...
    for (SizeType i = 0; i < NumNodes; ++i)
    {
    double norm_Body_Force=norm_2(BodyForce);
      if( this->GetGeometry()[i].SolutionStepsDataHas(BODY_FORCE) and norm_Body_Force !=0){ // it must be checked once at the begining only
	  // Build RHS

	      for (SizeType d = 0; d < TDim; ++d)
	    {
	      // Body Force
	      rRHSVector[FirstRow+d] += Weight * Density * rN[i] * BodyForce[d];
	    }
	    }
      else{
